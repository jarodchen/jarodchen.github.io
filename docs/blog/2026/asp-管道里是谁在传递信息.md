...
很多人第一次接触 ASP.NET Core 中间件的时候，会以为管道里流动的是 `HttpContext`，就像水管里流的水一样。

这个理解不能说全错，但比喻不够准。

更准确的说法是：**管道是河道，请求和响应数据是水，`HttpContext` 是随水漂流的容器——一个背包、船舱或者集装箱。中间件就是河道上的一个个关卡，它们检查水、加工水，也往容器里塞东西。**

所以 `HttpContext` 不是水本身。水是 `Request` 和 `Response` 里的内容。`HttpContext` 是把这些内容，连同用户、终结点、服务、特性、会话等等，全部装在一起的那个上下文对象。
